...
list_exceptions() {
	...
	find ../Formula -name "ksysroot_*.rb" | while IFS= read -r file; do
		...
		formula="${formula%.rb}"
    if [ "${formula}" = "ksysroot_native" ]; then
      continue
    fi
...
fetch_ksysroot() {
  ...
  KSYSROOT_SHA=$(${SHASUM} -a256 ${CACHE_DIR}/ksysroot.tar.gz | cut -d" " -f 1)
...
emit_package() {
  ...
  if [ "${KSYSROOT_TRIPLE}" = "native" ]; then
    ...
    case "${MESON_SYSTEM}" in 
      freebsd)
        if [ "${KSYSROOT_OSFLAVOUR}" = "freebsd" -a "${KSYSROOT_OSRELEASE}" = "14.2-RELEASE" ]; then
          lpackage="${package}"
        ...
        ;;
      linux)
        if [ "${KSYSROOT_OSFLAVOUR}" = "debian" -a "${KSYSROOT_OSRELEASE}" = "12" ]; then
          lpackage="${package}"
...
EOF
  ...
  if [ "${MESON_SYSTEM}" = "linux" ] && [ "${MESON_CPUFAMILY}" = "x86_64" ]; then
    cat >> "${package_file}" <<EOF
  on_linux do
...
EOF
  ...
  while read -r pkg version url file file_sha256; do
...
if [ -d "${BUILD_DIR}"/new ]; then
  echo "Processing new entries"
  branch="prm_new"
  git checkout -b "${branch}"
  find "${BUILD_DIR}/new" -name "*.rb" | while IFS= read package; do
    base=$(basename "${package}")
    echo new ${base}
    if [ -e "${package%.rb}.alias" ]; then
  ...
  git checkout main
  git branch -d "${branch}"
elif [ -d "${BUILD_DIR}"/mod ]; then
  echo "Processing modified entries"
  find "${BUILD_DIR}/mod" -name "*.rb" | while IFS= read package; do
    base=$(basename "${package}")
    echo make ${base}
    if [ -e "${package%.rb}.alias" ]; then
      branch="prp_${base%.rb}"
  ...
  ${KSYSROOT_DIR}/ksysroot.sh iterate | grep native | while IFS= read -r flavour; do
    ...
    if [ -e "${old_package_file}" ]; then
      unbottle bottle "${old_package_file}" | unrevision > "${new_package_file}.org"
      if diff -sq "${new_package_file}".org "${new_package_file}"; then
        ...
        if [ "${KSYSROOT_URL_OLD}" = "${KSYSROOT_URL}" ]; then
